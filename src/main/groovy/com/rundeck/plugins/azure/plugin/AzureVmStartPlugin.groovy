 ...
 */
@Plugin(name = AzureVmStartPlugin.PROVIDER_NAME, service = ServiceNameConstants.WorkflowStep)
class AzureVmStartPlugin  implements StepPlugin, Describable {
    public static final String PROVIDER_NAME = "azure-vm-start-step";
    public static final String PROVIDER_TITLE = "Azure / VM / Start"
    public static final String PROVIDER_DESCRIPTION ="Start an Azure Virtual Machine"
    ...
    public static final String VM_START_ASYNC="StartAsync"
    ...
    static Description DESCRIPTION = DescriptionBuilder.builder()
            ...
            null,null,null, renderingOptionsConfig))
            .property(PropertyUtil.bool(VM_START_ASYNC,"Start asynchronously ?","Will start the VM asynchronously ",
    ...
    void executeStep(PluginStepContext context, Map<String, Object> configuration) throws StepException {
        ...
        String clientId=configuration.get(AzureVmStartPlugin.CLIENT)
        String tenantId=configuration.get(AzureVmStartPlugin.TENANT)
        String subscriptionId=configuration.get(AzureVmStartPlugin.SUBSCRIPTION_ID)
        String keyPath=configuration.get(AzureVmStartPlugin.KEY)
        String pfxCertificatePath=configuration.get(AzureVmStartPlugin.PFX_CERTIFICATE_PATH)
        String pfxCertificatePassword=configuration.get(AzureVmStartPlugin.PFX_CERTIFICATE_PASSWORD)
        ...
        String name=configuration.get(AzureVmStartPlugin.VM_NAME)
        String resourceGroup=configuration.get(AzureVmStartPlugin.VM_RESOURCE_GROUP)

        String key = AzurePluginUtil.getPasswordFromKeyStorage(keyPath,context);
        Boolean async = Boolean.valueOf(configuration.get(AzureVmStartPlugin.VM_START_ASYNC))
        ...
        log.add(["date":t1.format("yyyy/MM/dd HH:mm"),"info":"Starting VM"])

        context.getExecutionContext()
                .getExecutionListener()
                .log(2,JsonOutput.toJson(log),meta);

        manager.startVm(name,async)

        Date t2 = new Date();

        log.clear()
        log.add(["date":t2.format("yyyy/MM/dd HH:mm"),"info":"VM started after ${((t2.getTime() - t1.getTime()) / 1000)} seconds"])
